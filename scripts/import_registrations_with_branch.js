...
async function importRegistrations(csvFilePath) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const uhid = row['registrationno'] || row['uhid'] || row['regno'] || `UHID-${Date.now()}-${i}`;
    ...
    const mobile = (row['mobileno'] || row['mobile'] || row['phone'] || '0000000000').replace(/\D/g, '').slice(0, 10) || '0000000000';
    const address = row['address'] || 'N/A';
    const state = row['statename'] || row['state'] || 'Delhi';
    ...
    const guardianName = row['guardianname'] || `${firstName}'s Guardian`;
    ...
    try {
      ...
      if (successCount % 100 === 0) {
        console.log(`Progress: ${successCount} patients imported...`);
      }
    } catch (err) {
...
const targetFile = process.argv[2] || '/home/srisir/Downloads/registrationslistcurrent.csv';
